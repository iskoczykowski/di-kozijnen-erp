...
function defaultTasks(lang: Lang): Task[] {
  return [
    ...
    { id: makeId('task'), title: lang === 'de' ? 'Produktion Auftrag A-2026-1042 starten' : 'Productie order A-2026-1042 starten', status: 'open', priority: 'normal', date: '01.07.' },
    { id: makeId('task'), title: lang === 'de' ? 'Lieferung zu Auftrag De Vries planen' : 'Levering voor order De Vries plannen', status: 'open', priority: 'normal', date: '02.07.' },
    { id: makeId('task'), title: lang === 'de' ? 'Auftrag A-2026-1041 überprüfen' : 'Order A-2026-1041 controleren', status: 'done', priority: 'normal', date: '28.06.' },
    { id: makeId('task'), title: lang === 'de' ? 'Rechnung Nr. R-2026-105 versenden' : 'Factuur nr. R-2026-105 verzenden', status: 'done', priority: 'normal', date: '28.06.' },
    { id: makeId('task'), title: lang === 'de' ? 'Montagematerial prüfen' : 'Montagemateriaal controleren', status: 'done', priority: 'normal', date: '29.06.' },
  ];
}
...
function defaultEvents(lang: Lang): EventItem[] {
  return [
    ...
    { id: makeId('ev'), title: lang === 'de' ? 'Besprechung – Produktion' : 'Bespreking – Productie', customer: '', location: lang === 'de' ? 'Büro' : 'Kantoor', date: '2026-06-30', timeFrom: '16:00', timeTo: '17:30', type: 'production' },
  ];
}

function defaultNotifications(lang: Lang): NotificationItem[] {
  return [
    { id: makeId('not'), icon: '🏠', title: T[lang].createdOrder, text: lang === 'de' ? 'Auftrag A-2026-1047 wurde erstellt.' : 'Order A-2026-1047 is aangemaakt.', time: '10:20', unread: true, type: 'order' },
    { id: makeId('not'), icon: '💬', title: T[lang].newMessage, text: lang === 'de' ? 'Von Familie Jansen' : 'Van familie Jansen', time: '10:15', unread: true, type: 'message' },
    ...
    { id: makeId('not'), icon: '📅', title: T[lang].montageTomorrow, text: lang === 'de' ? 'Aufmaß bei Familie Jansen um 09:00 Uhr.' : 'Inmeting bij familie Jansen om 09:00 uur.', time: '09:30', unread: true, type: 'montage' },
    { id: makeId('not'), icon: '🚚', title: T[lang].deliveryPlanned, text: lang === 'de' ? 'Lieferung für Auftrag A-2026-1045 am 02.07.' : 'Levering voor order A-2026-1045 op 02.07.', time: '08:45', unread: true, type: 'delivery' },
    { id: makeId('not'), icon: '✅', title: T[lang].orderDone, text: lang === 'de' ? 'Auftrag A-2026-1040 wurde abgeschlossen.' : 'Order A-2026-1040 is afgerond.', time: lang === 'de' ? 'Gestern' : 'Gisteren', unread: false, type: 'success' },
    { id: makeId('not'), icon: '🏭', title: T[lang].newProductionTask, text: lang === 'de' ? 'Produktion Auftrag A-2026-1042 gestartet.' : 'Productie order A-2026-1042 gestart.', time: lang === 'de' ? 'Gestern' : 'Gisteren', unread: false, type: 'production' },
    { id: makeId('not'), icon: '⚙️', title: T[lang].systemUpdate, text: lang === 'de' ? 'Version 2.1.0 wurde erfolgreich installiert.' : 'Versie 2.1.0 is succesvol geïnstalleerd.', time: lang === 'de' ? 'Gestern' : 'Gisteren', unread: false, type: 'system' },
  ];
}
...
const input: React.CSSProperties = { height: 42, border: '1px solid #d7e0ec', borderRadius: 12, padding: '0 12px', background: '#fff' };

function CalendarPanel({ lang, events, addEvent }: { lang: Lang; events: EventItem[]; addEvent: () => void }) {
  ...
  return (
    <section style={card}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', borderBottom: '1px solid #e2e8f0' }}>
        {['Mo', 'Di', 'Mi', 'Do', 'Fr', 'Za', 'Zo'].map((d, i) => <div key={d} style={{ padding: 8, textAlign: 'center', color: i >= 5 ? '#dc2626' : '#0f172a', fontWeight: 800 }}>{d}</div>)}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)' }}>
        {cells.map((c, i) => {
          ...
          return (
            <button key={`${c.key}-${i}`} style={{
              height: 44, border: '1px solid #edf2f7', background: isToday ? '#2563eb' : isHoliday ? '#fee2e2' : isWeekend ? '#fff1f2' : '#fff',
              color: isToday ? '#fff' : isHoliday || isWeekend ? '#dc2626' : c.current ? '#0f172a' : '#94a3b8',
              borderRadius: isToday ? 10 : 0, fontWeight: isToday ? 900 : 700, cursor: 'pointer', position: 'relative',
            }}>
              {c.day}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 18, alignItems: 'center' }}>
        <h3 style={{ margin: 0 }}>{T[lang].appointmentsFor} {selected.toLocaleDateString(lang === 'de' ? 'de-DE' : 'nl-NL', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' })}</h3>
      ...
      <div style={{ display: 'grid', gap: 8, marginTop: 12 }}>
        {dayEvents.map((e) => (
          <div key={e.id} style={{ display: 'grid', gridTemplateColumns: '110px 1fr', gap: 12, padding: 12, border: '1px solid #e2e8f0', borderRadius: 12 }}>
            ...
            <div><b>{e.title}</b><div style={{ color: '#64748b' }}>{e.location}</div></div>
          </div>
        ))}
      </div>
...
function TodoPanel({ lang, tasks, setTasks }: { lang: Lang; tasks: Task[]; setTasks: React.Dispatch<React.SetStateAction<Task[]>> }) {
...
export default function DashboardModule({ lang = 'de', setModule }: { lang?: Lang; setModule: (m: Module) => void }) {
  ...
  useEffect(() => { if (tasks.length) localStorage.setItem(TASK_KEY, JSON.stringify(tasks)); }, [tasks]);
  useEffect(() => { if (events.length) localStorage.setItem(EVENT_KEY, JSON.stringify(events)); }, [events]);
  useEffect(() => { if (notifications.length) localStorage.setItem(NOTIFICATION_KEY, JSON.stringify(notifications)); }, [notifications]);

  function addEvent() {
    const title = prompt(lang === 'de' ? 'Titel vom Termin:' : 'Titel van afspraak:');
    if (!title) return;
    setEvents((prev) => [...prev, { id: makeId('ev'), title, customer: '', location: T[lang].office, date: '2026-06-30', timeFrom: '09:00', timeTo: '10:00', type: 'customer' }]);
  }
    ...
    <div style={app}>
      <aside style={sidebar}>
        ...
        {(Object.keys(moduleLabels) as Module[]).map((id) => (
          <button key={id} title={moduleLabels[id][lang]} style={id === 'dashboard' ? iconActive : iconBtn} onClick={() => setModule(id)}>{moduleLabels[id].icon}</button>
        ))}
      </aside>
